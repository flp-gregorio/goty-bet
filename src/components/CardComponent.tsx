...
const CardComponent: React.FC<CardComponentProps> = ({ nominee, aux = "NULL", genre = "", background, active, setActiveCard }) => {
  ...
  return (
    <div
      className={`relative overflow-hidden aspect-square md:w-52 container text-left group hover:translate-x-3 hover:translate-y-3  transition-all duration-300 cursor-pointer 
        ${active ? 'bg-black translate-x-3 translate-y-3 grayscale' : 'bg-white grayscale-0'}
      `}
      onClick={handleCardClick}
      style={{ backgroundImage: `url(${background})`, backgroundSize: 'cover'}}
    >
      <div className={`p-3 w-full h-full ${active ? 'bg-white text-black' : 'bg-black text-white'} bg-opacity-40 transition-all duration-300`}>
